...
class QACNN():
    ...
    def __init__(self, sequence_length, batch_size,vocab_size, embedding_size,filter_sizes, num_filters, model_name, dropout_keep_prob=1.0,l2_reg_lambda=0.0,paras=None,learning_rate=1e-2,embeddings=None,loss="pair",trainable=True):
        ...
        with tf.name_scope("embedding"):
            if self.paras==None:
                if self.embeddings ==None:
                    ...
                    self.Embedding_W = tf.get_variable("{}_embedding_W".format(model_name),
                                                       shape=[vocab_size, embedding_size], initializer=initializer)
                else:
        ...
        for i, filter_size in enumerate(self.filter_sizes):
            with tf.name_scope("conv-maxpool-%s" % filter_size):
                ...
                if self.paras==None:
                    W = tf.get_variable("{}_kernel_W_{}".format(model_name, filter_size), shape=filter_shape,
                                        initializer=initializer)
                    b = tf.get_variable("{}_kernel_b_{}".format(model_name, filter_size), shape=[self.num_filters],
                                        initializer=initializer)
